...
# On my Raspberry Pi, 
globalKnownFacesDir = "known_faces"
...
class Video_Camera :
    ...
    def __init__ (self, cameraNum, preKnownFaces = [], preSafeNames = [], settings = { "recTolerance" : 0.55,
        ...
        self.knownFacesDir = globalKnownFacesDir
        
        self.errorMessage = cv2.imread("ErrorImage.jpeg")

        # List of dictionaries designed like so:
        # [{"PersonName" : [LIST_OF_FACE_ENCODINGS]}]
        self.knownFaces = preKnownFaces
        
        # Is a list of people that are considered "safe"
        self.safeNames = preSafeNames
    ...
    def loadFaces (self) :
        counter = 0
        for name in os.listdir(self.knownFacesDir) :
            ...
            self.knownFaces.append({name : []})

            # Loops through known faces folder
            for profile in os.listdir(f"{self.knownFacesDir}\\{name}") :
                print(profile)

                image = face_recognition.load_image_file(f"{self.knownFacesDir}\\{name}\\{profile}")
                ...
                self.knownFaces[counter][name].append(prof_encodings[0])

                # Confirming whether or not the face is illegible
                invalidFace = False

            if (invalidFace == True) :
                self.knownFaces.remove({name : []})
                continue
    ...
    def loadNames (self) :
        #try :
        # Loops through known faces folder
        for file in os.listdir(self.knownFacesDir) :
            if (file.endswith(".json") == True) :
                fileData = open(os.path.join(self.knownFacesDir, file), "r")
                ...
                if (personData["auth"] == "safe") :
                    self.safeNames.append(personData["name"])
    ...
    def patrol (self) :
        ...
        try :
            ...
            self.current_image = self.errorMessage
        ...
        while time.time() - start < self.confirmationTime :
            new_frame = self.video.read()[1]
            ...
            for face_encoding in encodings :
                ...
                for facePair in self.knownFaces :
                    #print(facePair)
                    name = list(facePair.keys())[0]
                    person_encodings = list(facePair.values())[0]
                    
                    results = face_recognition.compare_faces(person_encodings, face_encoding, tolerance=self.recTolerance)
        ...
        if (potentialPerson["name"] == True) :
            ...
            # print(tempratio)
            if (diffRatio > self.imageDifferenceTolerance) :
        ...
        if (potentialPerson["name"] in self.safeNames) :
            status = "Safe"
    ...
    def certifyResult(self, results) :
        ...
        for result in results :
            if (result == True) :
                goodResults += 1
                if (goodResults / length >= self.certifyTolerance) :
                    return True
    ...
    def getCommonGuy(self, resultHistogram) :
        ...
        if (positivePersonName != False) :
            ...
            if (positivityRatio > self.minPercOfPosImages) :
